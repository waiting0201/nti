#!/usr/bin/env bash
# 將 master 的新 commit 逐一「去掉 reference/」後接到 public 分支上。
#
# 為什麼需要這支腳本：git 無法對不同 remote 過濾路徑，同一分支推到哪裡內容都一樣。
# reference/（設計 PSD 與客戶素材，約 2.5GB）只能進 NAS，不能進 GitHub，
# 因此維持兩條分支：master（完整）→ Remote_NAS；public（無 reference）→ Remote_GitHub。
#
# 作法：用 plumbing 重建 tree，不動工作目錄（毫秒級，不會複製 2.5GB）。
...
n=0
for c in $commits; do
    # 取 master 該 commit 的根 tree，濾掉頂層的 reference 項目後重建
    tree=$(git ls-tree "$c" | grep -v "$(printf '\t')$EXCLUDE\$" | git mktree)
...
echo "推送：git push Remote_GitHub ${DST}:main"
