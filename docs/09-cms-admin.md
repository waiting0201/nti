# 09 · 後台 CMS — 功能規格書

| 欄位 | 內容 |
|------|------|
| **主責 Agent** | `backend-engineer`（API + 後台 SPA）／`system-analyst`（權限與欄位定義） |
| **對應階段** | P4（CMS）／P6（表單） |
| **技術基線** | 後台為 **純 SPA（靜態、noindex）**，資料一律走 `/api/v1/admin/*`（Azure Functions .NET 10 + Dapper） |
| **配套文件** | [08-database.md](08-database.md)（資料表）、[04-api.md](04-api.md)（契約）、[05-seo.md](05-seo.md)（SEO 欄位） |
| **資料來源** | `mockup/` 44 頁實際結構（2026-09-01 定案版）、[官網資訊架構 IA](../reference/官網資訊架構_IA.md) |

---

## 1. 三條專案決議（本文件的裁切依據）

1. **後台以單元功能為主** — 一個前台內容區塊 = 一個後台單元，各自獨立的清單／編輯畫面。不做通用 page-builder、不做區塊拖拉組版。
2. **不做 Media Library** — 沒有「媒體管理」選單。圖片／檔案只能從**所屬欄位**上傳，隨該筆資料建立與刪除；**每個上傳欄位旁必須顯示建議尺寸提示**（§3 為權威文字）。
3. **固定文字區不做後台** — 品牌敘述型的長篇文案寫死在前端（清單見 §7），後台只保留這些頁的 SEO 欄位。
   **2026-09-24 修訂**：About Us 五頁改為可在後台逐段改字（單元 15 的「頁面文字」，見 §7.1）；
   同日稍晚擴大到**所有有前台路由的固定頁**（28 頁，`green-csr` 無路由除外）。
   仍不做 page-builder——版面、段落數、圖片照 mockup，只有「字」可以換。

---

## 2. 後台選單與單元總表

| 群組 | # | 單元 | 代號 | 主要資料表 | 階段 |
|------|---|------|------|-----------|------|
| 首頁 | 01 | 首頁 Banner | `home-banner` | `HomeBanner` | P4 |
| 內容 | 02 | 解決方案 Solutions | `solution` | `Solution`／`SolutionItem` | P4 |
| 內容 | 03 | 案例實績 Projects | `project` | `Project` | P4 |
| 內容 | 04 | 最新消息 News | `news` | `News` | P4 |
| 內容 | 05 | Green Vlog | `vlog` | `Vlog` | P4 |
| 內容 | 06 | FAQ | `faq` | `Faq` | P4 |
| 內容 | 07 | 產業趨勢 Industry Trends | `trend` | `IndustryTrend` | P4 |
| 內容 | 08 | 認證・夥伴・獎項 | `certification` | `Certification` | P4 |
| 內容 | 09 | 客戶 Logo | `client` | `ClientLogo` | P4 |
| 內容 | 10 | 設備與廠房 | `facility` | `FacilityItem` | P4 |
| 內容 | 11 | 職缺 Careers | `job` | `JobPosting` | P4 |
| 供應商 | 12 | 供應商公告 | `supplier-notice` | `SupplierNotice` | P4 |
| 供應商 | 13 | 規格與要求 | `supplier-spec` | `SupplierSpec` | P4 |
| 供應商 | 14 | 下載專區 | `supplier-download` | `SupplierDownload` | P4 |
| 頁面／SEO | 15 | 頁面設定與 SEO | `page` | `Page` | P4 |
| 頁面／SEO | 16 | 舊網址轉址 | `redirect` | `Redirect` | P8 |
| 表單 | 17 | 報價需求 | `quote` | `QuoteRequest` | P6 |
| 表單 | 18 | 聯絡訊息 | `contact` | `ContactMessage` | P6 |
| 系統 | 21 | 網站設定 | `setting` | `SiteSetting` | P4 |
| 系統 | 22 | 分類管理 | `category` | `Category` | P4 |
| 系統 | 23 | 管理員與角色 | `admin` | `AdminUser`／`Role` | P4 |
| 系統 | 24 | 信件紀錄 | `audit` | `EmailLog` | P4 |
| 系統 | 25 | 消息標籤 | `tag` | `Tag`／`TagI18n`／`NewsTag` | P8 |


### 2.1 與規劃書 §3-1 功能清單的差異

規劃書（2026-06）的 CMS 功能清單有 22 項，與本文件的單元逐項比對後，**只有下列不是一對一對應**，其餘 13 項照收：

| 規劃書項目 | 現況 | 原因 |
|---|---|---|
| 首頁 COURAGE 區塊維護 | 移除 | 2026-09-01 首頁改版後 COURAGE 併入 Banner 標語，屬固定文案（§7） |
| 首頁 Project 精選維護 | 移除 | Featured Projects 整組已被 Proof 認證牆取代（[IA §6](../reference/官網資訊架構_IA.md)）→ 改由單元 08 維護 |
| NTI Difference 維護 | 拆分 | 敘述文字固定（§7）；認證／夥伴／獎項 → 單元 08 |
| Advantages 綠色優勢維護（圖文數據） | 改為固定 | 四個 Sustainability 內頁為品牌長文，含水處理數值等（§7） |
| 首頁 Banner 支援**影片**上傳 | 僅圖片 | mockup hero 為圖片輪播；要影片需追加欄位與播放器 |
| Green Vlog／**電子報** | 僅 Vlog | 電子報未納入，見下方缺口 |
| **電子報訂閱名單管理** | 未納入 | 見下方缺口 |
| **CSR 社會責任維護** | 未納入 | mockup 44 頁中無 CSR 頁 |
| 語系管理（中英對照） | 不設獨立單元 | 改為每個單元的中／英分頁 + 清單完成度 badge（§5.3） |

規劃書未列、本文件新增的單元，來自 2026-08-31 IA 改版與 SEO 需求：
`faq`、`trend`、`job`、`page`、`redirect`、`category`、`audit`、`order`。

> ⚠️ **三個待客戶確認的缺口**（在規劃書內、mockup 外）：**電子報**（含訂閱名單管理與舊站名單遷移）、**CSR 頁**、**首頁 Banner 影片**。
> 三者屬範圍變更，**未確認前不納入本期估算**（後台單元、前端與工時皆不含）。
>
> **schema 已先預留**（2026-09-02），客戶點頭後不必再動資料結構，只需補後台單元與權限碼：
>
> | 缺口 | 已預留 | 位置 |
> |---|---|---|
> | 電子報 | `NewsletterSubscriber` 表（double opt-in、`Source='Import'` 支援舊站名單匯入） | [08-database.md §4.15](08-database.md) |
> | 首頁 Banner 影片 | `HomeBanner.MediaType` / `VideoPath` + 兩條 CHECK | [08-database.md §4.2](08-database.md) |
> | CSR 頁 | `Page` 第 29 筆 `green-csr`（`HasRichBody = 1`、`IsIndexable = 0`） | [08-database.md §6.4](08-database.md) |

---

## 3. 上傳欄位與建議尺寸（**權威文字，逐字顯示在欄位旁**）

| 單元 | 欄位 | 欄位旁提示文字 |
|------|------|---------------|
| `home-banner` | 桌機圖 | 建議 **2400×900px**（16:6）｜JPG／WebP｜≤500KB |
| `home-banner` | 手機圖（選填） | 建議 **1080×1350px**（4:5）｜JPG／WebP｜≤300KB｜未上傳則沿用桌機圖 |
| `setting` | 首頁形象圖帶 | 建議 **2400×1000px**（12:5）｜JPG／WebP｜≤500KB |
| `solution` | 方案封面 | 建議 **1160×940px**（約 5:4）｜JPG／WebP｜≤300KB｜同時用於首頁 Printing Solutions 卡片 |
| `solution` | 品項圖 | 建議 **1280×960px**（4:3）｜JPG／WebP｜≤300KB |
| `project` | 案例圖 | 建議 **1160×940px**（約 5:4）｜JPG／WebP｜≤300KB |
| `news` | 封面圖 | 建議 **1800×1200px**（3:2）｜JPG／WebP｜≤400KB |
| `news` | 內文插圖（編輯器內） | 建議寬度 **1800px**（3:2 最佳）｜JPG／WebP｜≤400KB |
| `certification` | 認證 Logo | 建議 **600×600px 去背 PNG 或 SVG**｜主體置中滿版、四周留白請先裁掉｜≤100KB |
| `client` | 客戶 Logo | 建議 **短邊 ≥300px 去背 PNG 或 SVG**｜≤100KB |
| `facility` | 設備照 | 建議 **1200×1200px**（1:1）｜JPG／WebP｜≤300KB |
| `vlog` | 縮圖覆蓋（選填） | 建議 **1280×720px**（16:9）｜未上傳將自動取 YouTube 官方縮圖 |
| `page`／`news`／`solution` | OG 分享圖（選填） | 建議 **1200×630px**（1.91:1）｜JPG／PNG｜≤300KB｜未上傳則沿用封面圖｜**僅此三個單元有此欄位**（§5.6；對應 `Page`／`News`／`Solution` 的 `OgImagePath`） |
| `supplier-notice` | 公告附件（選填） | PDF｜≤20MB |
| `supplier-download` | 下載檔 | PDF／XLSX／DOCX／ZIP｜≤20MB｜檔案類型與大小由系統自動帶入前台 |
| （前台）`get-a-quote` | 設計稿上傳 | PDF／AI／PSD／JPG／PNG／ZIP｜單檔 ≤20MB｜最多 5 個檔案 |

**共通規則**
- 每個圖片上傳欄位**必附一個多語 `Alt` 文字欄位**（中／英各一，必填）— 05-seo 硬性要求。
  適用範圍是**頁面上呈現的 `<img>`**。三個例外在欄位宣告處以 `altExempt` 寫明理由，不是靜默豁免：
  客戶 logo（以品牌名稱作為 alt，單元 09 明訂）、OG 分享圖（`og:image` 是 meta 標籤，
  頁面上沒有這張圖）、vlog 縮圖覆蓋（緊鄰影片標題的裝飾性圖片，前台以 `alt=""` 呈現，
  補 Alt 會讓螢幕閱讀器把標題唸兩遍）。
- **選好檔案不會立刻送出**：後台只做本機預覽，真正的上傳在按下儲存時與其他欄位一起發生
  （2026-09-10 改；理由見下方變更紀錄）。因此欄位旁的「已選擇 …」與按鈕旁的「有未儲存的變更」
  講的是同一件事的兩半——檔案還在瀏覽器裡。
- **按下「移除」（或換掉一張圖）並存檔後，檔案就真的刪掉了**，不是只從這一格拿掉：
  留著的話那個檔案仍取得自它原本的網址，網址知道的人照樣抓得到。因此移除是不可復原的，
  再用回同一張圖要重新上傳。同一個檔案若還被別筆資料或另一個語系的內文引用，則會保留
  （2026-09-10 起；先前要等夜間清除，最久 7 天）。
- 上傳後由後端驗副檔名白名單 + 檔頭 magic number，另存 WebP 衍生檔供前台使用（原檔保留）。
  圖片與文件是**兩支端點**：`/admin/{unit}/upload` 收 JPG／PNG／WebP／SVG（≤10MB）、
  `/admin/{unit}/upload-file` 收 PDF／DOCX／XLSX／ZIP（≤20MB）。上表給 `supplier-notice`
  的「PDF」比 API 的白名單窄，那條較嚴的規則由後台介面的 accept 承擔。
- 超過建議尺寸不擋，但後台顯示黃色提醒「圖片 3200×1200，超過建議尺寸，將由系統壓縮」。
- 尺寸不足一半時擋下並提示重新上傳（避免前台糊圖）。
- ⚠️ 認證 logo 現有素材僅約 152×165px（見 [IA §6](../reference/官網資訊架構_IA.md)），**上線前需向客戶索取向量原檔**。

---

## 4. 單元規格

> 以下每個單元的欄位表中：**語** 欄位標 ✓ 表示中英各存一份；**必** 表示必填。共用操作與 UI 規則見 §5。

### 01 `home-banner` 首頁 Banner
**前台**：`index.html` `#hero` 輪播（目前 3 張）。
**操作**：新增／編輯／拖曳排序／上下架／刪除。**建議 3–5 張**，超過 6 張時提示。

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| 桌機圖 | 圖片上傳 | ✓ | | 見 §3 |
| 手機圖 | 圖片上傳 | | | 見 §3 |
| 圖片替代文字 | 單行 | ✓ | ✓ | |
| 連結網址 | 單行 | | | 站內相對路徑或完整外部 URL |
| 另開視窗 | 開關 | | | 預設關 |
| 排序 | 拖曳 | | | |
| 上架／上架時間／下架時間 | 狀態組 | | | 見 §5.4 |

### 02 `solution` 解決方案
**前台**：`solutions.html` 列表、`products-boxes/cardboard/uv/other.html` 四個方案頁、首頁 Printing Solutions 四張卡。
**操作**：**固定 4 筆，不可新增／刪除**（新增方案屬改版範圍）；可編輯、排序、上下架。品項卡為子清單，可自由增刪。

主檔欄位：

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| 代號 Code | 唯讀 | | | `boxes`／`cardboard`／`uv`／`other` |
| 方案名稱 | 單行 | ✓ | ✓ | 選單與卡片標題 |
| 頁面標題 H1 | 單行 | ✓ | ✓ | 如 Custom Color Box Packaging |
| 短述 | 多行(300) | | ✓ | 首頁／列表卡片用 |
| 方案導言 | 富文本 | | ✓ | 方案頁開頭段落 |
| 方案封面 + Alt | 圖片上傳 | ✓ | Alt ✓ | 見 §3 |
| SEO 欄位組 | — | | ✓ | 見 §5.6 |

品項子清單（`SolutionItem`）：品項圖 + Alt、名稱、說明（400 字內）、排序、上架。

### 03 `project` 案例實績
**前台**：`projects.html` `#cases` 卡片（可依分類篩選）。

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| 分類 | 下拉 | ✓ | | 來源 `category`（Project） |
| 案例圖 + Alt | 圖片上傳 | ✓ | Alt ✓ | 見 §3 |
| 標題 | 單行 | ✓ | ✓ | |
| 說明 | 多行(400) | | ✓ | |
| 影片連結 | 單行 | | | 有值才顯示播放圖示 |
| 數據值／數據說明 | 單行 | | 說明 ✓ | 如 `-32%` ／ `carbon / unit`；兩者需同時填或同時空 |
| 排序、上下架 | — | | | |

### 04 `news` 最新消息
**前台**：`news.html` 列表 + 12 支詳細頁；`insights.html` 精選；首頁（若啟用）。
**清單**：可依分類／狀態／關鍵字篩選，欄位含縮圖、標題、分類、發佈日、狀態、**中/英完成度 badge**。

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| 分類 | 下拉 | ✓ | | 來源 `category`（News） |
| 發佈日期 | 日期 | ✓ | | 前台顯示用（2026.03.13） |
| 封面圖 + Alt | 圖片上傳 | ✓ | Alt ✓ | 見 §3 |
| 標題 | 單行(250) | ✓ | ✓ | 同時作為 H1 |
| 摘要 | 多行(500) | ✓ | ✓ | 列表卡片 + 詳細頁導言 |
| 內文 | 富文本 | ✓ | ✓ | 允許：H3 小標、段落、粗體、清單、連結、圖片、引言 |
| 上首頁／精選 | 開關 | | | |
| OG 圖 | 圖片上傳 | | | 見 §3 |
| SEO 欄位組 | — | | ✓ | 見 §5.6 |

### 05 `vlog` Green Vlog

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| YouTube 影片 ID／網址 | 單行 | ✓ | | 貼完整網址系統自動抽出 ID，並即時顯示縮圖預覽 |
| 分類 | 下拉 | ✓ | | 來源 `category`（Vlog） |
| 標題／說明 | 單行／多行 | 標題 ✓ | ✓ | |
| 縮圖覆蓋 | 圖片上傳 | | | 見 §3 |
| 設為頁面主打影片 | 單選開關 | | | **全站僅一支**；開啟新的會自動關閉舊的 |
| 排序、上下架 | — | | | |

### 06 `faq` FAQ
分類（選填，來源 `category`）／問題（單行 300）／答案（富文本）／排序／上架。清單支援依分類分組拖曳。
> 前台需輸出 `FAQPage` JSON-LD（05-seo §2.3），故問答需為純文字可解析結構，答案富文本避免使用表格。

### 07 `trend` 產業趨勢
`industry-trends.html` 的段落區塊：標題（單行 200）／內文（富文本）／排序／上下架。目前 5 筆。

### 08 `certification` 認證・夥伴・獎項
**前台**：首頁 Proof 認證牆、`about-certifications.html`、`differences.html`、`green-esg.html`。

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| 分組 | 下拉 | ✓ | | 認證／合作夥伴／獎項（來源 `category`） |
| Logo + Alt | 圖片上傳 | ✓ | Alt ✓ | 見 §3 |
| 名稱／說明 | 單行／多行 | 名稱 ✓ | ✓ | |
| 連結網址 | 單行 | | | |
| 證號／取得日期／最近稽核日期 | 單行／日期／日期 | | | 2026-09-24 新增（GEO：AI 只引用具體數字）。有填就顯示在 `about-certifications` 認證牆該 logo 下方，並寫進 JSON-LD `hasCredential` |
| 顯示於首頁 Proof 牆 | 開關 | | | 預設開；首頁建議 12–16 枚，超過時提示 |
| 排序、上架 | — | | | |

### 09 `client` 客戶 Logo
名稱（同時作為 alt，品牌名不翻譯）／Logo 上傳／連結／排序／上架。前台為輪播，建議 ≥6 筆。

### 10 `facility` 設備與廠房
**前台**：`facility-pre-press` ／ `-eco-printing` ／ `-post-press` ／ `-quality` ／ `-tour` 五個子頁的設備卡。

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| 所屬子頁 | 下拉 | ✓ | | 印前／環保印刷／印後／品質檢驗／工廠導覽（來源 `category`） |
| 設備照 + Alt | 圖片上傳 | ✓ | Alt ✓ | 見 §3 |
| 設備名稱／說明 | 單行／多行(600) | 名稱 ✓ | ✓ | |
| 排序、上架 | — | | | |

> 五個子頁的**導言與流程敘述文字為固定**（§7），後台只管設備卡。

### 11 `job` 職缺
職稱（單行 160）／地點（單行）／說明（富文本）／排序／上下架（含自動下架日）。
> `careers.html` 的「Why NTI」六條福利為固定文案（§7）。

### 12 `supplier-notice` 供應商公告
分類（Policy／ESG／Quality／Logistics）／公告日期／標題／內文（富文本）／附件（PDF）／上下架。清單依日期倒序。

### 13 `supplier-spec` 規格與要求
標題（單行 160）／說明（多行 600）／排序／上架。目前 4 筆。

### 14 `supplier-download` 下載專區

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| 檔案 | 檔案上傳 | ✓ | | 見 §3 |
| 顯示名稱 | 單行 | ✓ | ✓ | 如「Supplier Handbook 2026 (EN)」 |
| 檔案類型／大小 | 唯讀 | | | 上傳時自動帶入，前台顯示 `PDF｜2.4 MB` |
| 下載次數 | 唯讀 | | | |
| 排序、上架 | — | | | |

### 15 `page` 頁面設定與 SEO
**29 筆固定頁**（28 筆既有 + 預留的 `green-csr`；清單見 [08-database.md §6.4](08-database.md)），**不可新增／刪除**，只可編輯。

| 欄位 | 型別 | 必 | 語 | 說明 |
|------|------|----|----|------|
| 頁面／路徑 | 唯讀 | | | |
| Slug | 單行 | ✓ | ✓ | 小寫、連字號；中英可不同 |
| SEO 標題 | 單行(70) | ✓ | ✓ | 即時字數計 + 超長警示 |
| 搜尋摘要 Meta Description | 多行(180) | ✓ | ✓ | 同上 |
| Canonical | 單行 | | ✓ | 留空則自動 |
| 社群分享標題／描述／圖片 | — | | 文字 ✓ | 留空則沿用 SEO 欄位 |
| 允許索引 | 開關 | | | 關閉 → `noindex` |
| 頁面內容 | 富文本 | | ✓ | **僅 `HasRichBody = 1` 者顯示此欄位**：`privacy-legal`，以及預留的 `green-csr` |

**頁面文字**（2026-09-24；2026-09-25 改版）：開放的頁面在「中文／English」分頁裡多出該頁每一段文字
（標題、段落、清單項、按鈕字、圖片替代文字），依區塊分組，排在頁面內容之後、SEO 欄位之前，只顯示目前語系。
**欄位直接填著前台目前顯示的字**，改了就是改了；清空則回到原文。DB 只存「與原文不同」的值，
所以沒動過的段落在原文日後修改時會跟著走。與 SEO 表單是不同的 API，但共用同一顆「儲存」。細節與限制見 §7.1。

### 16 `redirect` 舊網址轉址
舊網址（唯一、小寫）／轉到新網址／轉址方式（永久 301／暫時 302／永久・表單頁專用 308）／啟用／已轉址次數（唯讀）。支援 **CSV 匯入匯出**（舊站 46 頁 + 80 篇文章的對照表）。儲存時檢查轉址鏈與迴圈並擋下。

**出貨就帶著舊站的 227 條對照**（`db/content/210_legacy_redirects.sql`，由
`tools/build-redirect-sql.mjs` 從前台 middleware 的同一份對照表產生），客戶不必自己貼。
整批更新走本單元的 CSV 匯入（`db/content/210_legacy_redirects.csv`）。

> ⚠ 兩件目前與規格不符的事：**轉址鏈與迴圈的檢查還沒實作**（`AdminRedirectHandler.Validate`
> 只做必填、值域與正規化），而**前台還不讀這張表**——middleware 讀的是編譯進去的
> `apps/web/src/lib/legacy-redirects.ts`，所以客戶現在改這一頁不會影響正式站。

### 17 `quote` 報價需求
**前台來源**：`get-a-quote.html`。**唯讀資料 + 可改狀態**，不可編輯客戶填寫內容。

- **清單**：報價單號、公司、聯絡人、產品類型、狀態、送出時間；可依狀態／日期區間／關鍵字篩選。
- **詳細**：完整表單內容（公司／聯絡人／Email／電話／產品類型／產業／數量／尺寸／材質偏好／期望日期／永續建議勾選／需求描述）＋ 設計稿附件下載。

  > **附件不做病毒掃描**（2026-09-08 決策）。原規格是「掃毒未通過者不提供下載」，但本期不接掃描服務——
  > 接 Defender for Storage 是每月固定成本（Standard Node ≈ US$10／storage account，另計 $0.15/GB），
  > 與這個案子的規模不成比例。閘留著而沒有東西會把狀態改成 `Clean`，結果是後台永遠下載不到任何附件。
  > 改以三件事壓風險：下載限超管（`quote.download`）、後端一律以 `application/octet-stream` + `nosniff`
  > 送出（不讓瀏覽器直接開啟 PDF／SVG），後台介面明示「未經掃描，請存檔後用防毒軟體確認再開」。
  > 日後要接掃描服務，需重新加回 `QuoteAttachment.ScanStatus` 欄位與回寫事件入口。
- **可改**：狀態（待處理／處理中／已報價／已結案／垃圾訊息）、承辦人、內部備註、標記已回覆。
- **匯出**：依篩選結果匯出 CSV（權限 `quote.export`，匯出行為寫入 `audit`）。
- **通知**：送出時寄業務通知信 + 客戶確認信，寄送結果記於 `EmailLog`，後台可查看失敗原因並重寄。

### 18 `contact` 聯絡訊息
同 17 的模式，欄位為姓名／Email／公司／電話／訊息；狀態為 待處理／已回覆／已結案／垃圾訊息。
可改的欄位同 17：狀態、**承辦人**、內部備註、標記已回覆（`ContactMessage.AssigneeId`
於 2026-09-08 補上，先前只有 `QuoteRequest` 有這個欄位，後台的承辦人選了存不進去）。

### 19 `member` ／ 20 `order`（已移出範圍）

> **2026-09-06 移出專案範圍。** 客戶 2026-08-31 版 sitemap 沒有會員節點，`mockup/` 44 頁也沒有
> 會員中心畫面。原本規劃的「會員管理」與「訂單與生產進度」兩個單元、對應的資料表、
> 9 支前台會員 API 與 9 支後台 API 全數移除。**編號 19／20 保留不再使用**，
> 其餘單元維持原編號，避免既有交叉引用失效。

### 21 `setting` 網站設定
單頁表單，依 `SiteSetting` 的固定 key 清單渲染（[08-database.md §6.3](08-database.md)），分四組：

- **公司資訊**：公司名稱、地址、營業時間（多語）；電話、傳真、Email、Google Map 嵌入碼。
- **社群**：Facebook／LinkedIn／YouTube 網址（留空則前台不顯示該圖示）。
- **首頁**：形象圖帶圖片 + Alt。
- **信件**：報價通知收件者、聯絡通知收件者、密件副本（支援多組，逗號分隔，儲存時驗格式）。

> **值從哪來**：種子（EF 的 `HasData`／`db/seed/130_site_setting.sql`）只建 15 個 key、值留 NULL；
> 專案裡已有依據的 11 個值由 [`db/content/220_site_setting.sql`](../db/content/README.md) 匯入，
> 客戶不必把公司資訊重打一遍。仍留 NULL 的 4 個是**客戶還沒提供**：傳真、LinkedIn、YouTube、密件副本
> （社群留空前台就不顯示該圖示，所以 NULL 是有意義的狀態，不是漏掉）。
>
> **前台吃哪些**（2026-09-10 接上）：聯絡頁的地址／電話／Email／營業時間／地圖、footer 的社群圖示、
> 首頁形象圖帶與 Alt。改了設定重新整理就會變。`mail.*` 由後端寄信時讀。
>
> Google 地圖欄位存的是**網址**：貼整段 `<iframe>` 也可以，存檔時只留 `src`（前台自己組 iframe，
> 資料庫裡就不會有一段能注入任意 HTML 的值）。
>
> 中文值與 CMS 內容同一個狀態——**初稿，上線前須客戶校閱**；公司中文名沒有依據，中英兩欄都放英文法定名稱。

### 22 `category` 分類管理
以 `CategoryType` 分頁籤管理九類分類（News／Project／Vlog／FAQ／認證／設備／供應商公告／產業別／報價材質）。欄位：代號（建立後唯讀）、名稱（多語）、排序、啟用。
**已被引用的分類不可刪除**，只能停用；刪除前顯示引用筆數。
> 產業別同時供 `projects.html` 的 Industries 清單與報價表單下拉使用；報價材質供報價表單下拉使用。

### 23 `admin` 管理員與角色
管理員：帳號／通知信箱／顯示名稱／角色／啟用／最後登入；**密碼由建立者直接設定**。**登入失敗不鎖定帳號**（客戶 2026-09-09 決定移除：持續用錯誤密碼打某個帳號就能把真正的管理員鎖在外面，而攻擊者不必知道任何密碼）；暴力破解由登入端點的 reCAPTCHA v3 擋。
**帳號（`Username`）不限定 email 格式**、建立後唯讀（改帳號等於換一個人，稽核紀錄會對不上）；
**密碼在新增視窗直接輸入（至少 6 碼、需輸入兩次），系統不寄啟用信、也不產生亂數密碼**（客戶 2026-09-09 決定）；
帳密由建立者當場轉交，建立出來的帳號不強制首登改密碼。忘記密碼時在編輯視窗重設一組新的
（`PUT /admin/admin/{id}/password`，權限 `admin.edit`），同樣不寄信。通知信箱純粹是系統通知用，與登入無關。
角色：三個系統角色（超級管理員／內容編輯／檢視者，不可刪）＋ 權限矩陣勾選（§6）。
**不可停用或降級自己**；系統至少保留一名啟用中的超級管理員。
刪除管理員是**真的刪除**，刪掉後同一個帳號名可以重新建立；此人建立／修改過的內容不受影響，只是 `CreatedBy` 不再對得回一個帳號。（2026-09-09 起全站的刪除都是真刪，見 §5.7；單元 23 只是先改的那一個。）

### 24 `audit` 信件紀錄
唯讀。列出 `EmailLog`：時間／收件者／主旨／狀態／失敗原因，失敗者可重寄（`audit.resend`）。可依狀態篩選。

> 操作紀錄（`AuditLog`）已於 2026-09-06 移出本期範圍，單元代號與權限碼 `audit.*` 沿用給信件紀錄。

---

## 5. 共用 UI 與行為規則

### 5.1 清單頁
分頁（預設 20 筆）、關鍵字搜尋、狀態篩選、**中/英完成度 badge**、批次上／下架、逐列刪除與批次刪除（皆為真刪，見 §5.7）。有 `SortOrder` 的單元提供拖曳排序，排序結果即時儲存。

### 5.2 編輯頁
左側為語系中性欄位（圖片、日期、分類、狀態），右側為 **中文／English 分頁**的文字欄位。切換語系不離開頁面、不遺失未存內容。離開前有未儲存變更時攔截。

### 5.3 多語
- 儲存時允許只填一種語系（存草稿），但**上架時必須兩語系皆完整**，否則擋下並指出缺漏欄位。
- 「複製中文到英文」按鈕，供先上架後翻譯的情境。
- 清單每列顯示中／英完成度 badge，工具列另有 **「中英」篩選**（全部語系／英文未填／中文未填），
  用來把還沒補完的內容挑出來。範圍限單一單元，因此數字與清單一致；
  篩選開啟時清單取整份資料而非當頁，可排序的單元在篩選期間停用拖曳（順序不完整時拖曳會洗掉沒顯示的列）。

### 5.4 上下架
`上架` 開關 + `上架時間` + `下架時間`。三者組合的顯示狀態：草稿／已排程／上架中／已下架。時間以台北時間輸入、存 UTC。前台不快取 CMS 資料，存檔後重整就會看到（docs/02 §渲染）。

### 5.5 富文本編輯器
白名單標籤：`p h3 h4 strong em ul ol li a blockquote img figure figcaption br`。
禁止：`script iframe style` 與行內 `style`（YouTube 影片一律走 `vlog` 單元的欄位，不從編輯器貼 iframe）。
貼上 Word／網頁內容時自動清除樣式。伺服器端二次 sanitize，不信任前端。

### 5.6 SEO 欄位組（`page`／`news`／`solution`）
網址代稱 Slug、SEO 標題（70 字提示）、搜尋摘要 Meta Description（180 字提示）、指定標準網址 Canonical、社群分享標題／描述／圖片。
Slug 由標題自動產生、可手改；重複時擋下。已上架內容改 slug 時，**自動在 `redirect` 建立一筆 301**。

### 5.7 刪除
**一律真刪**（2026-09-09 改，原為軟刪）。刪除鈕在每一列都有一顆，勾選後另有批次刪除；確認對話框明說「無法還原」，並顯示前台影響（例如「此分類有 8 篇新聞引用」）。

改的理由：軟刪的資料從清單消失、前台也查不到，卻仍佔著 slug、分類代號這些唯一鍵，同名重建被擋成「已存在」——對操作者而言就是「刪了卻沒刪掉」。管理員（單元 23）早一步改真刪也是同一個原因。

- 中英文字（`*I18n`）、方案的品項卡、報價附件跟著主檔一起刪（DB 的 `ON DELETE CASCADE`）；報價附件的檔案本體也會從 Blob 移除
- **被引用的資料刪不掉**，理由要說出來：分類與標籤仍有內容掛著時只能停用；已有報價引用的方案刪不掉
- 開放刪除的範圍：內容單元 01–14、轉址、分類、標籤、管理員為原有；**15 固定頁、17 報價、18 聯絡訊息的刪除為 2026-09-09 新增，且只給超管**（固定頁刪掉之後前台那一頁就沒有 SEO 設定可讀；報價與聯絡是客戶送出的紀錄）

### 5.8 驗證與安全
必填／長度／格式（Email、URL、YouTube ID）皆前後端雙重驗證；後端為準。後台 SPA 以 `X-Robots-Tag: noindex` 排除索引，所有 `/api/v1/admin/*` 需 JWT + RBAC 檢查。

---

## 6. 權限矩陣

| 動作 | 超級管理員 | 內容編輯 | 檢視者 |
|------|:---:|:---:|:---:|
| 內容單元 01–14 檢視 | ✓ | ✓ | ✓ |
| 內容單元 01–14 新增／編輯／排序 | ✓ | ✓ | — |
| 內容單元 01–14 上下架 | ✓ | ✓ | — |
| 內容單元 01–14 刪除 | ✓ | ✓ | — |
| 15 頁面 SEO ／ 16 轉址 | ✓ | ✓ | 檢視 |
| 15 頁面：刪除 | ✓ | — | — |
| 17 報價 ／ 18 聯絡：檢視・改狀態 | ✓ | ✓ | 檢視 |
| 17 報價：附件下載・匯出 CSV | ✓ | — | — |
| 17 報價 ／ 18 聯絡：刪除 | ✓ | — | — |

| 21 網站設定 ／ 22 分類 | ✓ | — | 檢視 |
| 23 管理員與角色 | ✓ | — | — |
| 24 信件紀錄 | ✓ | — | — |
| 25 消息標籤 | ✓ 檢視／編輯／刪除 | ✓ 檢視／編輯 | ✓ 檢視 |

權限碼 `{單元代號}.{view|edit|publish|delete|export}`，存於 `RolePermission`。上表逐格展開為種子列共 **173 列**（SuperAdmin 84／Editor 68／Viewer 21），見
 [`db/seed/110_role_permission.sql`](../db/seed/110_role_permission.sql)；**本表為權限的權威來源**，[08-database.md §6.1](08-database.md) 僅為摘要。

矩陣描述到、但原本未定代號的三項（2026-09-02 補）：

| 權限碼 | 對應矩陣列 | 角色 |
|---|---|---|
| `quote.download` | 17 報價：附件下載 | 僅 SuperAdmin |
| `redirect.export` | 16 轉址：CSV 匯入匯出 | SuperAdmin、Editor |
| `audit.resend` | 24 信件紀錄：`EmailLog` 重寄 | 僅 SuperAdmin |

2026-09-09 隨「刪除改真刪」新增的三項：

| 權限碼 | 對應矩陣列 | 角色 |
|---|---|---|
| `page.delete` | 15 頁面：刪除 | 僅 SuperAdmin |
| `quote.delete` | 17 報價：刪除 | 僅 SuperAdmin |
| `contact.delete` | 18 聯絡：刪除 | 僅 SuperAdmin |

`SuperAdmin` 亦逐列展開、不使用 `system.*` 之類的萬用碼——RBAC 檢查邏輯保持單一（一律查 `RolePermission`）且可稽核。

---

## 7. 不做後台的固定文字區（決議 3 的明細）

下列內容**寫死在前端程式碼**，修改需走改版流程（開發者調整 + 重新部署）。這些頁在後台仍有 `page` 單元可改 SEO。

| 頁面／區塊 | 內容 | 不做後台的理由 |
|---|---|---|
| 首頁 What We Do（4 格） | Structural Design／Pre-Press (CTP)／Printing／Finishing | 品牌簡報定案文案，屬品牌定位不隨檔期變動 |
| 首頁 Why global brands choose NTI?（6 格） | Direct Delivery … One Trusted Partner | 同上 |
| `green-our-advantage` / `green-carbon` / `green-materials` / `green-esg` 敘述段落 | 綠色永續長文（含水處理數值等） | 同上；數據更動屬改版等級 |
| `facility*` 五個子頁的導言與流程說明 | 製程敘述 | 同上（設備卡本身有後台，見單元 10） |
| `solutions` / `projects` / `insights` hub 頁的導言與導覽卡 | 區塊標題與引導文 | 結構性文案，與版型綁定 |
| `contact` / `get-a-quote` 表單標籤與說明文字 | 欄位標籤、隱私同意文字 | 與表單驗證邏輯綁定，改文案需同步改程式 |
| 各頁 banner 裝飾大圖 | 版型背景圖 | 屬視覺設計資產，非內容 |
| 頁首選單、頁尾連結、浮動按鈕 | 站台結構 | 由 IA 決定，改動＝改架構 |

> **擴充路徑**：若客戶日後要求自行維護上述任一段落，**首選是把該頁加進「頁面文字」**（§7.1，
> 不改 schema、不動版面）。整頁改成自由排版的 rich text（`HasRichBody = 1` 並改讀 `PageI18n.BodyHtml`）
> 只適合像隱私權聲明這種本來就是純文章的頁——套在有卡片、圖示格線的頁面上會失去版型。

### 7.1 頁面文字覆寫（2026-09-24：所有固定頁）

客戶要求 About Us 的文字可自行修改，首批開放 About Us 五頁；同日依客戶 2026-09-22 的 SEO/GEO 文案盤點
（綠色優勢、設備、方案總覽、首頁 H1 等都改不到），擴大為**所有有前台路由的固定頁**（`PageKeys` 扣掉
沒有路由的 `green-csr`，共 28 頁）。上方 §7 表格列的「寫死的段落」因此都能逐段改寫；§7 的定位
（不做 page-builder、版面照 mockup）不變。

**機制**：前台的 `<T>`（`apps/web/src/lib/translate.tsx`）本來就逐一走過頁面上的文字節點，拿英文原文
查中文字典；現在多一層「後台覆寫」，查表順序是 **覆寫 → 字典（zh.ts）→ 原文**。所以任何一段文字都能換，
而頁面結構一個字元都不必改。覆寫存在 `PageText`（[08 §4.11](08-database.md)），以英文原文為鍵。

**後台清單的來源**：`node apps/web/scripts/extract-page-texts.mjs` 從 mockup 抽出這幾頁的文字，產生
`apps/admin/src/api/page-texts.generated.ts`。mockup 或 zh.ts 改了要重跑。

**三處要一致**：`extract-page-texts.mjs` 的 `PAGES`、`Api/Common/Constants.cs` 的 `PageTextPages`
（＝`PageKeys.All` 扣掉 `green-csr`）、以及前台各頁的 `<T locale={locale} overrides={page?.texts}>`。
`build-pages.mjs` 產生的頁由產生器自動接線（依 `lib/pages.ts` 的 `PAGE_KEY_BY_PATH`），重跑不會洗掉；
`HAND_MAINTAINED` 的頁是手動接的。新增固定頁時三處一起補。

**限制**（已知、刻意接受）：

- **原文改了，覆寫就對不上。** 鍵是英文原文；日後在 mockup 改了某段英文並重跑 codegen，那段的覆寫就不再顯示。
  後台會把這些列成「原文已變更」，讓人把內容搬到新段落再移除——與 `zh.ts` 同一個性質。
- **同一段原文在同頁出現多次會一起改**（例如分頁標籤與區塊標題都是「The NTI Difference」），後台標示「×2」。
- **只能改寫、不能增刪**：段落數與條目數照 mockup。文件新增的一句話只能併進既有段落。
- **CMS 區塊不在清單內**：有資料時整塊換成後台單元的內容（首頁輪播／認證牆／客戶輪播、設備格線、
  案例、消息、FAQ、影片、趨勢、職缺、供應商三區、`about-certifications` 的認證牆），覆寫在那裡不會生效，
  所以 `extract-page-texts.mjs` 的 `CMS_BLOCKS`／`CMS_TEXTS` 把它們排除，要改請到對應單元。
- header／footer／浮動鈕不在此範圍（頁面的 `<T>` 包不到它們）。

---

## 8. DoD

> 僅列本文件獨有項；SEO／安全／效能的通則見 [`03-backend.md` §5](03-backend.md)。

- [ ] §2 的各單元皆可獨立操作，無跨單元耦合。
- [ ] **每個上傳欄位旁都顯示 §3 的建議尺寸提示文字**，且每個圖片欄位都有中英 Alt（`altExempt` 的三個例外除外，見 §3）。
- [ ] 無「媒體管理」選單；刪除資料列即解除檔案引用。
- [ ] §7 清單中的區塊確認為前端寫死，未誤做成後台欄位。
- [ ] 上架時強制中英兩語系完整，缺漏欄位明確指出。
- [ ] §2.1 的三個缺口已取得客戶書面確認（納入或不納入）。

---

## 變更紀錄

| 日期 | 修改者 | 摘要 |
|------|--------|------|
| 2026-09-01 | Tim（Claude Code） | 初版：依 mockup 44 頁與三條決議（單元式後台／無 Media Library／固定文字不進後台）定義 24 個後台單元、上傳尺寸總表、權限矩陣與固定文字清單 |
| 2026-09-02 | Tim（Claude Code） | §3 上傳總表的 OG 分享圖列由「各單元」更正為 `page`／`news`／`solution`（與 §5.6 及 `OgImagePath` 實際欄位一致）｜配合 [`db/`](../db/) 建置腳本落地：§2.1 三個待確認缺口補記「schema 已預留」對照表（範圍與估算不變）；§6 權限矩陣確立為權威來源並補 `quote.download`／`redirect.export`／`audit.resend` 三個權限碼、註明展開後共 171 列；§15 固定頁 28 → 29 筆（含預留的 `green-csr`），富文本欄位不再僅限 `privacy-legal` |

| 2026-09-06 | Tim（Claude Code） | §23 後台帳號不再限定 email 格式：登入識別改為 `Username`（建立後唯讀），信箱降為選填的通知欄位；密碼長度下限由 8 碼放寬為 6 碼（前後端各一處，`AuthHandler.MinPasswordLength` ↔ `Login.tsx` 的 `MIN_PASSWORD_LENGTH`） |
| 2026-09-06 | Tim（Claude Code） | **移除單元 19 會員管理與 20 訂單與生產進度**（客戶 2026-08-31 sitemap 無會員節點、mockup 44 頁無會員中心）。§2 表刪兩列、§19/§20 改為移除說明並保留節次編號、§6 權限矩陣刪一列（171 → 167 列，SuperAdmin 83 → 79）；§14 供應商下載刪除「需登入下載」欄位（受控文件概念一併取消） |
| 2026-09-06 | Tim（Claude Code） | **操作紀錄移出本期範圍**：單元 24 由「操作紀錄」改為 **信件紀錄**，只剩 `EmailLog`（狀態／失敗原因／重寄）。移除 §00 儀表板的「最近 20 筆操作紀錄」、§5.8 的「寫入操作記 `AuditLog`」。單元數與權限矩陣不變（22 個單元、167 列，`audit.view`／`audit.resend` 沿用） |
| 2026-09-08 | Tim（Claude Code） | **報價附件不做病毒掃描**：§17 原規格「掃毒未通過者不提供下載」改為無掃描的知情下載。接掃描服務是每月固定成本（Defender for Storage ≈ US$10／storage account），與案子規模不成比例；閘留著卻沒有東西會把狀態改成 `Clean`，結果是後台永遠下載不到附件。改以「限超管 + octet-stream/nosniff + 介面警語」承擔風險 |
| 2026-09-08 | Tim（Claude Code） | **後台三個「存不進去」的欄位分別處置**：§18 聯絡訊息補上承辦人（`ContactMessage.AssigneeId`，先前只有 `QuoteRequest` 有）；vlog 縮圖 Alt 與 OG 圖 Alt 從 UI 移除，§3 的「每個圖片欄位必附中英 Alt」收斂為「頁面上呈現的 `<img>`」，三個例外（客戶 logo／OG 圖／vlog 縮圖）在欄位宣告以 `altExempt` 寫明理由，§8 DoD 同步。另補報價清單的 CSV 匯出入口 |

| 2026-09-09 | Tim（Claude Code） | **新增單元 25 消息標籤**（客戶 2026-09-08 SEO 簡報的 CMS「標籤」欄位，以及內容遷移決策 D4「保留標籤體系並逐一 301 對應」）。`Tag`／`TagI18n`／`NewsTag` 三張表、前台封存頁 `/{語系}/news/tag/{slug}`、消息單元加標籤多選欄位。權限矩陣 167 → 173 列（SuperAdmin 79 → 82、Editor 67 → 69、Viewer 21 → 22）。舊站 100 個 `/tag/*` 收斂為 17 個新標籤，99 條 1:1 轉址 |
| 2026-09-09 | Tim（Claude Code） | **單元 16 301 轉址由隱藏改為啟用**，並補上 CSV 匯入的入口（後端 `POST /admin/redirect/import` 早就在，只是後台沒有按鈕）。匯入以 `fromPath` 為鍵覆寫，重跑同一份檔案不會產生重複 |
| 2026-09-09 | Tim（Claude Code） | **單元 23 的密碼改為直接設定**（客戶決定）：新增管理員時在視窗輸入密碼（兩次確認），編輯視窗可重設密碼；不再產生亂數初始密碼、不寄啟用信，建立出來的帳號也不強制首登改密碼。通知信箱降為純通知用途 |
| 2026-09-09 | Tim（Claude Code） | **單元 23 的刪除改為真刪**：原本走全站的軟刪，帳號從清單消失、也登不進來，但 `Username` 仍被佔住，同名重建被擋成「此帳號已存在」。§5.7 與 §23 同步，確認對話框改為說明「可以重新建立同名帳號」 |
| 2026-09-09 | Tim（Claude Code） | **移除單元 00 待辦總覽**：四張數字卡中「中英未對齊」與「7 天內即將下架」統計的是全站累加，點下去卻寫死跳最新消息，數字與落點對不上；且全站掃描在分頁 API 下（`pageSize=100`）超過 100 筆就會少算，是個會說謊的指標。§2 表刪一列、§4 刪 §00 節、§6 權限矩陣 173 → 170 列（刪 `dashboard.view`，SuperAdmin 82 → 81、Editor 69 → 68、Viewer 22 → 21）、`GET /admin/dashboard` 與 `AdminDashboardHandler` 一併移除（EF migration `DropDashboardPermission`）。**登入後改為導向側邊欄第一個有權限的單元**；中英完成度下放到各單元清單的「中英」篩選（§5.3），範圍限單一單元，數字就不會再失真 |

| 2026-09-09 | Tim（Claude Code） | **各單元的刪除改為真刪，並讓它找得到**（§5.1／§5.7 改寫）。兩件事：(a) 原本一律軟刪，資料從清單消失、前台也查不到，卻仍佔著 slug 與分類代號，同名重建被擋成「已存在」——對操作者就是「刪了卻沒刪掉」；(b) 刪除鈕只長在「勾選之後」的工具列上，客戶回報「後台沒有刪除功能」，因此每一列都補一顆刪除鈕。同時新增 `page.delete`／`quote.delete`／`contact.delete`（僅超管），§6 矩陣 170 → 173 列 |
| 2026-09-10 | Tim（Claude Code） | **單元 16 舊網址轉址填入實際資料**：`tools/build-redirect-sql.mjs` 從前台 middleware 的對照表產出 `db/content/210_legacy_redirects.{csv,sql}`（227 條＝舊站 sitemap 的 229 個網址扣掉兩條新舊相同的），後台 mock 的 7 筆示意資料改為這 227 條真資料。SQL 刻意「已存在的 FromPath 不覆蓋」——這張表客戶會編輯，重跑覆蓋等於默默改掉他調整過的落點 |
| 2026-09-10 | Tim（Claude Code） | **後台介面文字去技術化**：後台是給客戶編輯內容的人用的，畫面上不應出現實作名詞。移除／改寫了 webhook・ISR・UTC・JSON-LD・noindex・`HasRichBody = 1`・`SiteSetting` 的 key 清單・`VITE_API_BASE`・權限碼與 `db/seed` 路徑等字樣；單元的「前台位置」由 `index.html #hero` 這類檔名改為使用者看得到的頁面名稱（首頁 主視覺輪播）；SEO 欄位改為「網址代稱 Slug／SEO 標題／搜尋摘要／指定標準網址／社群分享標題・描述・圖片」；單元 16 更名為**舊網址轉址**，欄位改為舊網址／轉到新網址／轉址方式／已轉址次數。清單的選項欄位改為顯示選項文字而非存進資料庫的代號（報價與聯絡訊息的狀態原本直接印出 `New`／`Replied`）|
| 2026-09-10 | Tim（Claude Code） | **上傳改為按下儲存才送出**（§3 共通規則）。原本選好檔案當下就 POST 上去，於是「檔案已進 Blob」與「資料還沒存」被拆成兩個時間點：使用者選了圖卻沒存檔或直接關掉分頁，檔案就成了 DB 沒有任何引用的孤兒，要等 `OrphanMediaFunction` 過了寬限期才收。改成暫存於瀏覽器、儲存時才一併上傳，孤兒視窗從「使用者猶豫的幾分鐘」縮成「兩個請求之間的幾百毫秒」（Blob 與 DB 不同交易，孤兒清除仍要留著）。代價是格式／大小的錯誤會晚一步才發生，因此選檔當下就先在前端擋，白名單與後端 `UploadRules` 對齊。同時新增文件端點 `/admin/{unit}/upload-file`——原本唯一的 `/upload` 只收圖片，公告附件與供應商下載檔接上 API 後會一律被退 |

| 2026-09-10 | Tim（Claude Code） | **移除檔案改為當場刪除**（§3 共通規則）。原本按了「移除」只是把欄位清空，檔案留在儲存體上，仍取得自原本的網址——前台頁面確實不再顯示它，但「移除」的本意是它不該再被拿到。唯一的清除機制是夜間排程，預設還關著、而且有 7 天寬限期。改成存檔成功後就刪，刪前重掃一次引用（別筆資料或另一個語系的內文還在用就保留）。副作用是移除不可復原，欄位旁的提示（「按下儲存後，這個檔案會從系統刪除，無法復原」）與 §3 都寫明 |
| 2026-09-11 | Tim（Claude Code） | 前台改成不快取之後，把「ISR 3600 秒」「webhook 觸發 revalidate」兩處說明改掉——存檔後前台重整就是新的 |
| 2026-09-25 | Tim（Claude Code） | **編輯畫面改版**：「基本設定」「上下架」移到右欄，中英分頁的文字區在左。單元 15 的「頁面文字」併入中英分頁（頁面文字在上、SEO 在下），欄位直接顯示前台目前的字、只顯示當下語系；拿掉英文原文對照、灰字提示與「只顯示改過的段落」。儲存時與原文相同的值不存（DB 語意不變） |

*最後更新：2026-09-25*

