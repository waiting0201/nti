...
| **對應階段** | P4（後端/CMS）／P6（報價/聯絡表單）／P8（內容遷移） |
...
> 本期不含 AI 客服（Claude API），亦不含**會員系統與訂單／生產進度**（2026-09-06 移出範圍：客戶 sitemap 無會員節點，mockup 亦無會員中心頁面）。
...
| Functions 冷啟動影響 API | ⚠️ 2026-09-11 起前台不快取，**每個請求都會打 API**——原本寫的「走 ISR 不每次打 API」已不成立。冷啟動直接影響首位訪客的首屏，必要時評估 Flex/常駐 |
...
| 2026-09-06 | Tim（Claude Code） | **會員系統與訂單／生產進度移出專案範圍**：§3 刪除「會員管理」與「會員系統（P6）」；§4 表數 49 → 45、後台單元 24 → 22、權限 171 → 167 列；工作分解第 4 項由「會員與表單」改為「表單後端」 |
| 2026-09-11 | Tim（Claude Code） | 更正 §7 風險表：前台拿掉 ISR 之後，「內容頁走 ISR 不每次打 API」這條對策已經不存在，冷啟動與 DTU 的壓力直接落在每一個請求上（見 docs/02、docs/07） |
