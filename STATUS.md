...
> 判斷「哪些算孤兒」依賴那份欄位清單是否完整（`MediaReferenceScanner`）——新增 `*Path`
> 欄位卻忘了補進去，就會把正在用的圖當成孤兒。建議先看幾輪報告再打開。
>
> 它是**安全網不是主要機制**（2026-09-10）：編輯者移除或換掉的檔案由 `MediaCleaner`
> 在存檔成功後當場刪掉，刪前重掃一次引用。不然按了「移除」的檔案仍取得自
> `/files/media/{path}`，最久要等到寬限期過後的那次掃描——而排程預設還關著。
> 實測十種情境（換圖／刪整筆／同一張圖被兩筆引用／中英內文各自移除／OG 圖清空）皆如預期。
>
